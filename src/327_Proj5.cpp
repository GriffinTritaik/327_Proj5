...
#include "../../327_Proj5_Lib/includes/Watch.h"
#include "../../327_Proj5_Lib/includes/constants.h"
#include "../../327_Proj5_Lib/includes/Functions.h"

using namespace std;

//tracks how many points you will get out of 100
//starts at 25 since I don't have 25 more points
//worth of tests.
...
void testGiveAndTakeWatch(Smalltalk *st, Watch* pWatch) {
	Watch *pCopy_pWatch = pWatch;
	Watch *pTakenWatch = 0;
	
	st->giveWatch(pCopy_pWatch);
	EXPECT_CONTAINS(THE_CURRENT_TIME_IS, st->getTime(), "4 - gettime()", 2,
			true);
	
	//take the watch
	pTakenWatch = st->takeWatch();
	EXPECT_EQ(pTakenWatch, pWatch,
			" verify taken watch is same as given watch", 1);
	EXPECT_CONTAINS(I_DO_NOT_HAVE_A_WATCH, st->getTime(), "4 - gettime()", 2,
			true);

	//try to take again should return null
	pTakenWatch = st->takeWatch();
	bool bval = (pTakenWatch == 0);
	EXPECT_EQ(bval, true,
